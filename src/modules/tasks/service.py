...
async def create_task(session: AsyncSession, title: str, owner_id: int, description: Optional[str] = None) -> Optional[Task]:
...
async def toggle_task_completion(session: AsyncSession, task_id: int, user_id: int) -> Optional[Task]:
    task = await repository.get_by_id(session, task_id)
    if not task or task.owner_id != user_id:
        return None
